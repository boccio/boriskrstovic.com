...
class blu_flickr_widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		echo !empty($instance['title']) ? $before_title.$title . $after_title : '' ?>
  		<div class="widget-body"> 
  			<a class="username" href="http://www.flickr.com/photos/<?php echo $username; ?>" target="_blank"><?php echo $username; ?></a>
	...
	function form( $instance ) {
		...
		$html .= '<input type="text" id="' . $this->get_field_id( 'title' ) . '" name="' . $this->get_field_name( 'title' ) . '" value="' . $instance['title'] . '" style="width:100%;" />';
		$html .= '</p>';
		...
		$html .= '<input type="text" id="' . $this->get_field_id( 'user_id' ) . '" name="' . $this->get_field_name( 'user_id' ) . '" value="' . $instance['user_id'] . '" style="width:100%;" />';
		$html .= '</p>';

		$html .= '<p>';
		$html .= '<label for="' . $this->get_field_id( 'username' ) . '">' . __( 'Flickr Username:', 'bluth_admin' ) . '</label>';
		$html .= '<input type="text" id="' . $this->get_field_id( 'username' ) . '" name="' . $this->get_field_name( 'username' ) . '" value="' . $instance['username'] . '" style="width:100%;" />';
		$html .= '</p>';
