<?php
	...
	function blu_format_video( $post )  {  
	    ...
	    $video_iframe = get_post_meta( $post->ID, 'blu_video_iframe', true );  
	    $display_thumbnail = get_post_meta( $post->ID, 'blu_display_thumbnail', true );  
	    ...
	    <fieldset class="clearfix blu_format blu_format_video"> 
		    ...
		    <div class="cd-layout" style="width: 100%;"> 
		    	<textarea id="video-iframe" name="blu_video_iframe" value="" placeholder="<?php _e('Video Embed Code..', 'bluth_admin'); ?>" style="margin: 10px 0; width:100%; font-size: 16px;"><?php echo $video_iframe; ?></textarea>
		    </div>
		</fieldset>
		<?php
	}
	function blu_format_audio( $post )  {  
	    ...
	    $audio_iframe = get_post_meta( $post->ID, 'blu_audio_iframe', true );  
	    ...
	    <fieldset class="clearfix blu_format blu_format_audio"> 
		    ...
		    <div class="cd-layout" style="width: 100%;"> 
		    	<textarea id="audio-iframe" name="blu_audio_iframe" value="" placeholder="<?php _e('Audio Embed Code..', 'bluth_admin'); ?>" style="margin: 10px 0; width:100%; font-size: 16px;"><?php echo $audio_iframe; ?></textarea>
		    </div>
		</fieldset>
		<?php
	}
	...
	function bluth_post_meta( $post )  {  
	    ...
	    if(empty($custom_thumbnail['gallery_src'])){ $custom_thumbnail['gallery_src'] = ''; }
	    $fallback_image = '';
	    ...
	    $blu_image_in_header = get_post_meta( $post->ID, 'blu_image_in_header', true );
	    $blu_hide_featured = get_post_meta( $post->ID, 'blu_hide_featured', true );
	    ...
	    ?>  
	    <div class="clearfix">
		    <fieldset class="pull-left one-four"> 
				...
				<div class="blu_image_upload full">
					...
					<button type="button" class="close remove_image">Remove</button>
		      	</div>
		    </fieldset> 
		    <fieldset class="pull-left half"> 
			    <h2 style="display:block; border-bottom:1px solid #cccccc; margin:15px 0; padding: 5px 0;">
			    ...
			    <div class="cd-layout" style="width: 32%; margin-right: 1%;"> 
			    ...
			    <div class="cd-layout" style="width: 32%;"> 
		    ...
		    <fieldset class="pull-left one-four"> 
			    <h2 style="display:block; border-bottom:1px solid #cccccc; margin:15px 0; padding: 5px 0;">
			    	<?php _e('Featured Image Settings', 'bluth_admin'); ?>
			    	<small style="display: block; font-size: 11px; line-height: 1.4; opacity: 0.7;">Featured image settings</small>
			    </h2> 
			    <div class="cd-layout" style="width: 250px;"> 
			        <input type="checkbox" id="blu_image_in_header" name="blu_image_in_header" <?php checked( $blu_image_in_header, 'on' ); ?> />  
       				<label for="blu_image_in_header"><?php _e('Show Featured Image in Header', 'bluth_admin'); ?></label>  
			    </div> 
			    <div class="cd-layout" style="width: 250px;"> 
			        <input type="checkbox" id="blu_hide_featured" name="blu_hide_featured" <?php checked( $blu_hide_featured, 'on' ); ?> />  
       				<label for="blu_hide_featured"><?php _e('Hide Original Featured Image', 'bluth_admin'); ?></label>  
			    </div> 
			    <br><br>
		    </fieldset> 
	    </div>
	...
	{  
	    ...
	    $blu_custom_header_area_height = get_post_meta( $post->ID, 'blu_custom_header_area_height', true );  
	    $blu_image_in_page_header = get_post_meta( $post->ID, 'blu_image_in_page_header', true );
	    $blu_title_in_page_header = get_post_meta( $post->ID, 'blu_title_in_page_header', true );
	    $blu_hide_page_featured = get_post_meta( $post->ID, 'blu_hide_page_featured', true );
	    ...
	    <fieldset class="clearfix pull-left half"> 
	    	<h4 style="border-bottom:1px solid #cccccc; margin:25px 0;"><?php _e('Title & Image Options', 'bluth_admin'); ?></h4> 
		    <p class="clearfix"> 
		    ...
		    <div class="cd-layout clearfix" style="width: 250px;"> 
		    ...
		    <div class="cd-layout clearfix" style="width: 250px;"> 
		        <input type="checkbox" id="blu_title_in_page_header" name="blu_title_in_page_header" <?php checked( $blu_title_in_page_header, 'on' ); ?> />  
   				<label for="blu_title_in_page_header"><?php _e('Show Title in Header', 'bluth_admin'); ?></label>  
		    </div> 
		    <div class="cd-layout clearfix" style="width: 250px;"> 
		        <input type="checkbox" id="blu_image_in_page_header" name="blu_image_in_page_header" <?php checked( $blu_image_in_page_header, 'on' ); ?> />  
   				<label for="blu_image_in_page_header"><?php _e('Show Featured Image in Header', 'bluth_admin'); ?></label>  
		    </div> 
		    <div class="cd-layout clearfix" style="width: 250px;"> 
		        <input type="checkbox" id="blu_hide_page_featured" name="blu_hide_page_featured" <?php checked( $blu_hide_page_featured, 'on' ); ?> />  
   				<label for="blu_hide_page_featured"><?php _e('Hide Original Featured Image', 'bluth_admin'); ?></label>  
		    </div> 
		    <p class="clearfix"> 
		        <input type="text" id="blu_custom_header_area_height" name="blu_custom_header_area_height" placeholder="<?php _e('Custom Header Area Height (in pixels)', 'bluth_admin'); ?>" value="<?php echo $blu_custom_header_area_height; ?>" style="margin: 10px 0; width:100%; font-size: 16px;" />  
		    </p> 

		    <br><br>
	    </fieldset> 

	    <fieldset class="clearfix half"> 
	...
	{ 
	    ...
	    if( isset( $_POST['bluth_post_left_sidebar'] ) ) 
	 	...
	 	$chk = isset( $_POST['blu_image_in_header'] ) ? 'on' : 'off';  
	    update_post_meta( $id, 'blu_image_in_header', $chk );  

	 	$chk = isset( $_POST['blu_hide_featured'] ) ? 'on' : 'off';  
	    update_post_meta( $id, 'blu_hide_featured', $chk );  
	    ...
	    if( isset( $_POST['blu_video_iframe'] ) ) 
	        update_post_meta( $id, 'blu_video_iframe', esc_attr(  $_POST['blu_video_iframe'] ) );

	    if( isset( $_POST['blu_audio_url'] ) ) 
	        update_post_meta( $id, 'blu_audio_url', esc_attr(  $_POST['blu_audio_url'] ) );
	    if( isset( $_POST['blu_audio_iframe'] ) ) 
	        update_post_meta( $id, 'blu_audio_iframe', esc_attr(  $_POST['blu_audio_iframe'] ) );
	...
	{ 
	    ...
	    if( isset( $_POST['blu_custom_header_area_height'] ) ) 
	        update_post_meta( $id, 'blu_custom_header_area_height', esc_attr( strip_tags( $_POST['blu_custom_header_area_height'] ) ) );

	 	$chk = isset( $_POST['blu_title_in_page_header'] ) ? 'on' : 'off';  
	    update_post_meta( $id, 'blu_title_in_page_header', $chk );  

	 	$chk = isset( $_POST['blu_image_in_page_header'] ) ? 'on' : 'off';  
	    update_post_meta( $id, 'blu_image_in_page_header', $chk );  

	 	$chk = isset( $_POST['blu_hide_page_featured'] ) ? 'on' : 'off';  
	    update_post_meta( $id, 'blu_hide_page_featured', $chk );  
