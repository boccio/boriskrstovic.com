...
<?php
    ...
    if(bl_utilities::get_option('bl_post_comments')){
        ...
        if ( post_password_required() )
            return; ?>
        <div id="comments" class="clearfix"><?php
          ...
          $args = array(
            'id_form'           => 'commentform',
            'id_submit'         => 'submit',
            'title_reply'       => blu_boldsplit(__( 'Leave a Reply' , 'bluth' )),
...
</div><!-- #comments .comments-area --><?php

  // var_dump(of_get_option('facebook_app_id'));
if(of_get_option('facebook_app_id') and bl_utilities::get_option('bl_facebook_comments')){
    echo '<div id="fb-comments" class="comments-area comments-area-facebook">';
    echo '<div class="fb-comments" data-href="'.get_permalink().'" data-width="700" data-num-posts="10"></div>';
    echo '</div>';
} ?>